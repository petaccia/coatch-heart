...
import jwt, { VerifyOptions, JsonWebTokenError, TokenExpiredError } from 'jsonwebtoken';
...
// Database-aligned payload without temporal claims
type UserPayload = Omit<JwtPayload, 'iat' | 'exp'>;
...
export const authenticateJWT = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    return next(createError("Accès non autorisé. Token manquant ou invalide.", 401));
  }
  ...
  try {
    ...
    if (!userExists) {
      ...
      return next(createError("Token associé à un utilisateur inexistant.", 401));
    }
    ...
    if (error instanceof JsonWebTokenError) { // Gère signature invalide, malformé, etc.
